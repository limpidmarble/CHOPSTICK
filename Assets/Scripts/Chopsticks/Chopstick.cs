...
{
    public float stick_squeeze_angle; // 제일 기본적인 젓가락 변수입니다. 0도 (11자 상태) 에서 얼마나 기울어져있는지 표시합니다. (단위: 도).물체와 충돌해 젓가락 자체는 멈춰도 stick_squeeze_angle은 최대치까지 올라갈 수 있습니다.
    ...
    public float angle_on_collision; // 양 젓가락이 모두 물체와 닿아 더 이상 기울어질 수 없을 때의 앵글 값을 저장하는 변수입니다. 이 변수값과, 더 올라간 stick_squeeze_angle값과의 차이를 구해 힘값을 도출합니다.
    ...
    void Start()
    {
        ...
        stick_squeeze_angle = stick_squeeze_min_angle; // 젓가락의 기본 앵글은 stick_squeeze_min_angle로 설정
    ...
    void Update()
    {
        ...
        {
            stick_squeeze_angle += stick_squeeze_speed * Time.deltaTime; // 마우스 왼쪽 버튼을 누르고 있으면 stick_squeeze_angle이 초당 stick_squeeze_speed만큼 증가합니다. 
            if (stick_squeeze_angle > stick_squeeze_max_angle) // stick_squeeze_angle이 최대를 넘어가면
            {
                stick_squeeze_angle = stick_squeeze_max_angle; // 최대로 고정합니다.
            }
        }
        else
        {
            if (stick_squeeze_angle > stick_squeeze_min_angle) //stick_squeeze_angle이 최소보다 크면
                stick_squeeze_angle -= stick_squeeze_speed * Time.deltaTime; // 마우스 왼쪽 버튼을 누르고 있지 않으면 stick_squeeze_angle이 초당 stick_squeeze_speed 만큼 감소합니다.
            if (stick_squeeze_angle < stick_squeeze_min_angle) // stick_squeeze_angle이 최소보다 작아지면
            {
                stick_squeeze_angle = stick_squeeze_min_angle; // 최솟값으로 고정합니다.
            }
        }
            ...
            if (left_stick.is_touching_stick || right_stick.is_touching_stick) // 왼쪽, 오른쪽 젓가락이 닿아있으면
        {
            ...
            angle_on_collision = stick_squeeze_angle; // 현재 stick_squeeze_angle을 angle_on_collision에 저장합니다.
        }
            stick_left_axis.localRotation = Quaternion.Euler(0, 0, stick_squeeze_angle); // 왼쪽 젓가락 축의 회전값을 stick_squeeze_angle로 설정합니다.
            stick_right_axis.localRotation = Quaternion.Euler(0, 0, -stick_squeeze_angle); // 오른쪽 젓가락 축의 회전값을 -stick_squeeze_angle로 설정합니다. -인 이유는 오른쪽 젓가락이 왼쪽 젓가락과 반대 방향으로 회전해야 오므려지기 때문입니다.
        }
        else
        {
